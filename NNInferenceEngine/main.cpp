...
void print_usage(const std::string& program_name) {
    std::cerr << "Usage: " << program_name << " <model_folder_path> <input_file_path> <num_inference_runs>\n";
    std::cerr << "Example: " << program_name << " models_large models_large/input.csv 10000000\n";
}
...
int main(int argc, char* argv[]){
    ...
    //        }
        ...
        std::string model_folder_path = argv[1];
        std::string input_file_path = argv[2];
        int num_inference_runs = std::stoi(argv[3]); // Convert string to int

        // --- Neural Network Inference Pipeline Test ---
        ...
        try {
            ...
            std::cout << "\n--- Input Loading ---\n";
            Matrix input_data = NeuralNetwork::load_matrix_from_csv(input_file_path, 1, 128);

            std::cout << "\n--- Initial Inference (for detailed layer timings - prints once) ---\n";
            
            Matrix initial_output_for_timing = pipeline.run_inference(input_data, true); // Pass true to print layer timings
            std::cout << "Initial output sample: " << initial_output_for_timing.get_value(0,0) << std::endl;


            std::cout << "\n--- Performance Benchmarking ---\n";
            std::cout << "Running inference " << num_inference_runs << " times for total time measurement...\n";

            std::mt19937 gen(std::random_device{}());
            std::uniform_real_distribution<float> dis(-0.001f, 0.001f);

            auto start_inference_loop = std::chrono::high_resolution_clock::now();

            for (int i = 0; i < num_inference_runs; ++i) {
                input_data.set_value(0, 0, input_data.get_value(0, 0) + dis(gen));
                if (input_data.get_value(0,0) > 1.0f) input_data.set_value(0,0, 0.5f);
                if (input_data.get_value(0,0) < 0.0f) input_data.set_value(0,0, 0.5f);

                pipeline.run_inference(input_data, false); // Pass false to suppress layer timings
            }

            auto end_inference_loop = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> total_inference_duration = end_inference_loop - start_inference_loop;
            std::cout << "Total inference time for " << num_inference_runs << " runs: "
                      << total_inference_duration.count() << " seconds\n";

            double avg_time_per_inference_ms = (total_inference_duration.count() * 1000.0) / num_inference_runs;
            std::cout << "Average time per inference: " << avg_time_per_inference_ms << " ms\n";

            std::cout << "\n--- Final Output Verification ---\n";
            Matrix final_output = pipeline.run_inference(input_data, false); // No prints for this final run
            std::cout << "Final output sample: " << final_output.get_value(0,0) << std::endl;
            std::cout << "Verification: Output printed successfully. Accuracy verification needs a reference model.\n";
